...
export function generateStaticParams() {
  return getGuidesBySection('Developers').map((doc) => ({ slug: doc.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const doc = getGuideBySlug(params.slug);
  if (!doc) return {};
  const title = await getCmsPageTitle(doc.url ?? `/developers/${params.slug}`, doc.title);
  return {
  ...
  return (
    <article className="space-y-6">
      <header className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Developers</p>
